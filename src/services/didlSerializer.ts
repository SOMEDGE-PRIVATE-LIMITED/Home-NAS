 ...
 */
export function serializeToDidl(
  items: Array<MediaItem | Container>,
  filter: string,
  baseUrl: string
): string {
  ...
  const includeField = (key: string): boolean =>
    filterAll || (filterSet !== null && filterSet.has(key));
...
function serializeItem(
  ...
  let xml =
    `<item id="${xmlEscape(item.id)}" parentID="${xmlEscape(item.parentId)}" restricted="1">` +
...
function buildResAttributes(
  ...
  let attrs =
    ` protocolInfo="${xmlEscape(protocolInfo)}"` +
    ` size="${item.fileSize}"`;
...
function serializeContainer(container: Container): string {
  return (
    `<container id="${xmlEscape(container.id)}"` +
 ...
 */
function mimeToUpnpClass(mimeType: string): string {
  if (mimeType.startsWith('video/')) return 'object.item.videoItem';
  if (mimeType.startsWith('audio/')) return 'object.item.audioItem';
  if (mimeType.startsWith('image/')) return 'object.item.imageItem';
  return 'object.item';
}
 ...
 */
function buildDlnaFeatures(dlnaProfile?: string): string {
