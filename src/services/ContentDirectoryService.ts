...
import type { MediaIndex, MediaItem, Container } from '../media/MediaIndex.js';
import { parseSoapAction } from './soapParser.js';
import { serializeToDidl } from './didlSerializer.js';
 ...
 */
export function registerContentDirectoryService(
...
function handleBrowse(
  ...
  let items: Array<MediaItem | Container>;
  ...
  const didl = serializeToDidl(items, filter, baseUrl);
...
function handleSearch(
  ...
  const didl = serializeToDidl(paginated.slice, filter, baseUrl);
...
): { slice: T[]; totalMatches: number; numberReturned: number } {
 ...
 */
function buildBrowseResponse(
  actionName: 'Browse' | 'Search',
  didl: string,
